...
<ul>
        ...
        <li style="background-color:red"><a href="history.php">History</a></li>
...
<div class="login" style="margin-left:10%;margin-right:10%">
        ...
        <div style="color : red;text-shadow: 1px 1px grey;font-family:monospace; font-size:30px;font-weight:1000;">History List</div>
<?php 
          ...
          <table class='styled-table'>
            ...
            <?php  
                    ...
                    try {
					    ...
					    $sql ="SELECT * FROM `Ranking` ORDER By `date` DESC,`Status` DESC,idRanking DESC";
                        ...
                        if(mysqli_fetch_array($result2)){                     
                             while($row=mysqli_fetch_array($result)){
                                ...
                                //Button
                                if ($status==0) {
                                    $statusbutton="<button type='submit' value='$idRanking' name ='Announce' class='myButton green' >Announce</button>";
                                }else{
                                    $statusbutton="<button type='submit' value='$idRanking' name ='Disable' class='myButton red' >Disable</button>";
                                }   
                                //----------------
                                // Gather member name : 
                               ...
                               $result20 = mysqli_query($connect,$sql20);
                                ...
                                //----------------
                                    ...
                                    if($newRankR<20){
                                        if ($NewRank<$OldRank) { //promotion
                                            $History="<a>".$MemName." has been </a><a style='color:green'>Promoted</a> from ".$disOldRank." to ".$disNewRank." by <a style='font-weight:bold'>".$Admin."</a>";
                                        }elseif ($newRankR==10) {
                                                $History="<a>".$MemName." has </a><a style='color:red'>left</a> the crew";
                                        }else{
                                            $History="<a>".$MemName." has been </a><a style='color:red'>Demoted</a> from ".$disOldRank." to ".$disNewRank." by <a style='font-weight:bold;'>".$Admin."</a> ";
                                        }
                                    }else{
                                        $History="<a>".$MemName." has been </a><a style='color:red'>Kicked</a> from the crew by <a style='font-weight:bold'>".$Admin."</a>";
                                    }
                                ...
                                echo"<th>".$statusbutton."&nbsp;<button type='submit' value='$idRanking' name ='Delete' class='myButton red' >Delete</button></th>";
                        ...
                        }else{echo "<tr><td colspan='3'>History is empty</td></tr>";}
...
//---------------------------------------------- Retirer Classe ---------------------------------------
        ...
        if(isset($_POST['Delete'])) {
            ...
            $idRankingg=$_POST['Delete'];  //id Classe
            $sql="DELETE FROM `Ranking` WHERE `idRanking` LIKE '".$idRankingg."'";
            if(mysqli_query($connect,$sql)){

            $_SESSION['Msg']="<center><a style='color:#00b000;text-shadow: 1px 1px black;font-weight:1000;font-size:20px;font-family: monospace;'>History Updated!</a></center>";
...
//Disable
        if(isset($_POST['Disable'])) {
            ...
            $idRankingg=$_POST['Disable'];  //id Classe
            $sql="UPDATE `Ranking` SET `status` = '0' WHERE `Ranking`.`idRanking` = '".$idRankingg."'";
            if(mysqli_query($connect,$sql)){
...
//Announce
        if(isset($_POST['Announce'])) {
            ...
            $idRankingg=$_POST['Announce'];  //id Classe
            $sql="UPDATE `Ranking` SET `status` = '1' WHERE `Ranking`.`idRanking` = '".$idRankingg."'";
            if(mysqli_query($connect,$sql)){
                $_SESSION['Msg']="<center><a style='color:#00b000;text-shadow: 1px 1px black;font-weight:1000;font-size:20px;font-family: monospace;'>Annoncement Enabled!</a></center>";
                header('Location: history.php');
            }
        }
?>